...
# python -m real_world.single_realsense
if __name__ == '__main__':
    # Example usage
    # Create a SharedMemoryManager
    # This is a context manager that will automatically clean up the shared memory when done
    resolution=(1280,720)
    serial_number = "327122075831"
    ...
    with SharedMemoryManager() as shm_manager:
        cam = SingleRealsense(shm_manager, serial_number, resolution, enable_color=True)
        cam.start()
        # 等待初始化完成
        cam.start_wait()
        # 设置曝光
        cam.set_exposure(exposure=200)
        # 获取一帧数据
        vis_img = None
